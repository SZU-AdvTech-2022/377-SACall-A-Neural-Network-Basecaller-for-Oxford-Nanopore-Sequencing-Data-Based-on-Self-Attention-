"""
Bonito Fast5 Utils (最新 0.6.1 版本)
"""
...
class Read(reader.Read):
    ...
    def __init__(self, read, filename, meta=False, do_trim=True):
        ...
        self.shift, self.scale = reader.normalisation(self.scaled)
...
def get_meta_data(filename, read_ids=None, skip=False):
    """
    Get the metadata from the fast5 file for a given `filename`.
    """
    meta_reads = []
...
def get_read_groups(directory, model, read_ids=None, skip=False, n_proc=1, recursive=False, cancel=None):
    """
    Get all the read metadata for a given `directory`.
    """
    groups = set()
    num_reads = 0
